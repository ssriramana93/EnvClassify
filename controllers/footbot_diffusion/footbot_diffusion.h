...
private:
   ...
   Real backtrack_count = 0;
   bool prevR = false, prevL = false;
   int stuckcount = 0;


   bool alt = false;
   std::tuple<size_t, size_t, size_t, size_t> odd, even, podd, peven;
   std::vector<std::tuple<size_t, size_t, size_t, size_t> > nobs;
   ...
   size_t integration_window = 30;
   std::vector<std::pair<Real, Real> > wheel_movements;
   bool integration_disable = false;

   void moveleft(Real);
   void moveright(Real);
   void movestraight(Real);
   void movebehind(Real);
